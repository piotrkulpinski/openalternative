...
export const searchAlternatives = cache(
  async (searchParams: SearchParams, { where, ...args }: Prisma.AlternativeFindManyArgs) => {
...
export const findAlternativeSlugs = cache(
  async ({ where, orderBy, ...args }: Prisma.AlternativeFindManyArgs) => {
    return prisma.alternative.findMany({
      ...args,
      orderBy: orderBy ?? { name: "asc" },
...
export const findAlternative = cache(async ({ ...args }: Prisma.AlternativeFindUniqueArgs) => {
